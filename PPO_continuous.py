...
def train(env_name, env, state_dim, action_dim, render, solved_reward, print_interval,
    max_episodes, max_timesteps, update_timestep, action_std, K_epochs, eps_clip,
    gamma, lr, betas, ckpt_folder, restore, tb=False):
    ...
    for i_episode in range(1, max_episodes+1):
        ...
        if running_reward > (print_interval * solved_reward):
            print("########## Solved! ##########")
            torch.save(ppo.policy.state_dict(), ckpt_folder + '/PPO_continuous_solved_{}.pth'.format(env_name))
            break
...
def test(env_name, env, state_dim, action_dim, render, action_std, K_epochs, eps_clip, gamma, lr, betas, ckpt_folder, test_episodes):
    ...
    print('Avg episode reward: {} | Avg length: {}'.format(avg_episode_reward/test, avg_length/test_episodes))
...
if __name__ == '__main__':
    ...
    if opt.mode == 'train':
        train(env_name, env, state_dim, action_dim,
            ...
            print_interval=opt.print_interval, max_episodes=opt.max_episodes,
            max_timesteps=opt.max_timesteps, update_timestep=opt.update_timesteps,
        ...
        test(env_name, env, state_dim, action_dim,
            ...
            gamma=opt.gamma, lr=opt.lr, betas=[0.9, 0.990], ckpt_folder=opt.ckpt_folder, test_episodes=10)
    else:
